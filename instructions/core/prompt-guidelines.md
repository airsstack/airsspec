...
State exactly what to do. Do not assume the agent will infer intent. Use direct action verbs: READ, WRITE, CREATE, VERIFY, HALT, ASK.
...
>
> **REASON**: The Research phase produces requirements only. Code changes would bypass the architectural decision process (DAA → ADR → RFC) and create untraced implementations that violate the "Trust But Verify" principle.
...
Organize instructions with clear markdown headers. This creates unambiguous boundaries between concerns.

**Required sections for ALL instructions:**

| Section Header | Purpose | Required |
|----------------|---------|----------|
...
| `## Steps` or `## Actions` | Explicit numbered steps to perform | ✅ Yes |
...
| `## When Uncertain` | What to do when the situation is unclear |
| `## Examples` | Concrete examples of expected behavior |
| `## Quick Reference` | Summary table for quick scanning |

---

### 4. Permission for Uncertainty

Explicitly ALLOW agents to ask questions rather than assume. This reduces hallucination and improves accuracy.
...
For each phase, explicitly list ALLOWED tools with specific use cases and BLOCKED tools with clear reasons why.
...
Every instruction MUST have explicit success criteria. Use checkboxes that can be verified programmatically or by the agent.
...
Instructions should request the minimum necessary work. Do not encourage extra features, abstractions, or "improvements."
...
All state MUST be persisted to the filesystem. Agents should reference files, not conversation memory.
...
- Use markdown headers for section boundaries
- Use blockquotes for examples and callouts
...
| Use Structure | Markdown headers with clear boundaries |
